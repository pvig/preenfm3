...
- Additional oscillator warp-path micro-optimisations in `Osc.h`: unified warped index mapping through `getWarpedIndexFast`, reused cached wavetable fields (`max` and table pointer) in hot render paths, and removed now-unused helper routines after refactoring.
- Feedback oscillator path (`getNextBlockWithFeedbackAndEnveloppe`) was reorganised to hoist interpolation-mode branching out of per-sample loops, reducing branch pressure in the inner loop while preserving default DSP behaviour.
- Added compile-time switch `PFM3_FAST_FEEDBACK_INTERP` (default `0`) in `Osc.h` to allow a speed-first feedback mode that disables feedback-path interpolation (`1`), intentionally trading timbral accuracy for CPU headroom.
- Preset randomizer (`SynthState::randomizePreset`) overhauled with coherent cross-parameter behaviour and a wider musical range:
  - **Cross-parameter coherence**: the four randomizer controls (`Oper`, `EnvT`, `IM`, `Modl`) now influence each other — `EnvT` biases the algorithm selection (percussive patches favour feedback/carrier-heavy algos; pad patches favour complex modulator trees), oscillator mix is scaled by the `IM` level so modulator activity feels consistent, percussive mode can inject user waveforms (`USER1..USER6`) with low probability (10%), and pad mode uses the full waveform palette with a strong sine bias (85% sine / 15% non-sine).
  ...
  - **LFO coherence**: percussive patches assign one-shot LFO shapes (exponential/log decay, attack-decay, Buchla) at faster transient rates (`3..(10+3*Modl)` Hz) with phase stagger; pad patches use slow smooth continuous shapes with long keyboard ramps and can also run in slow internal multi-shot modes (2–8 cycles) for evolving modulation that settles.
  ...
  - **Envelope tail control / infinite-sustain prevention**: pad envelope times are capped (decay 0.5–3 s, sustain ramp 0.5–2 s, release 1–5 s, down from 5/5/8 s ceilings), percussive sustain/release were tightened (`0.02–0.5 s` / `0.1–2.5 s`) to avoid overly long drum tails, and `releaseLevel` is fixed at `0.0` for all envelope modes so voices always decay fully to silence. This also closes the modulator-loop edge case where `releaseLevel == 1.0` combined with `releaseTime == 0.0` could cause an envelope to loop indefinitely.
  - **LFO DC-offset safety**: percussive and random modes keep `osc->bias = 0.0`. Pad mode can use non-zero bias for asymmetric movement, but only when that LFO is not routed to amplitude-like destinations (`MIX_OSC*`, `ALL_MIX`, `FILTER*_AMP`); unsafe pad routings are rerouted to safe destinations before enabling offset.
